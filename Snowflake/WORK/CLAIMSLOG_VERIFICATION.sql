...
--     SELECT RECORD_ID FROM CPE_CLAIMS_LOG SAMPLE (100 ROWS)
    SELECT RECORD_ID FROM CPE_CLAIMS_LOG LIMIT 500
),
claim_log AS (
  SELECT l.*
...
combined AS (SELECT
     ...
     log.REQ_DV_OTHER_PAYER_AMOUNT_PAID                AS LOG_DV_OTHER_PAYER_AMOUNT_PAID,
     res.OTHER_PAYER_AMOUNT_PAID                        AS RES_OTHER_PAYER_AMOUNT_PAID,
     ...
     log.REQ_NR_OTHER_PAYER_PATIENT_RESPONSIBILITY_AMOUNT_COUNT,
     log.REQ_NP_OTHER_PAYER_PATIENT_RESPONSIBILITY_AMOUNT_QUALIFIER,
     log.REQ_NQ_OTHER_PAYER_PATIENT_RESPONSIBILITY_AMOUNT,

     log.RES_5F_APPROVED_MESSAGE_CODE_COUNT,
     log.RES_6F_APPROVED_MESSAGE_CODE
...
--
--      -- Other Amount Paid
--     ,LOG_RES_OTHER_AMOUNT_PAID_QUALIFIER
--     ,RES_OTHER_AMOUNT_PAID_QUALIFIER
--
--     ,LOG_RES_OTHER_AMOUNT_PAID -- Currency Precision
--     ,RES_OTHER_AMOUNT_PAID
--
--
--     // Currency Array
--     ,LOG_REQ_OTHER_PAYER_AMOUNT_PAID_QUALIFIER
--     ,REQ_OTHER_PAYER_AMOUNT_PAID_QUALIFIER
--
--     ,LOG_DV_OTHER_PAYER_AMOUNT_PAID
--     ,RES_OTHER_PAYER_AMOUNT_PAID
--
--     // Approved Message Code
--     ,LOG_RES_APPROVED_MESSAGE_CODE
--     ,RES_APPROVED_MESSAGE_CODE
--
--     // Other Payer Patient
...
--     ,REQ_NQ_OTHER_PAYER_PATIENT_RESPONSIBILITY_AMOUNT
     ...
     ,RES_5F_APPROVED_MESSAGE_CODE_COUNT
     ,RES_6F_APPROVED_MESSAGE_CODE
    ,*


FROM SAMPLE_DATA
-- WHERE ARRAY_SIZE(LOG_RES_OTHER_AMOUNT_PAID_QUALIFIER) > 1;
-- WHERE TRY_TO_NUMBER(RES_OTHER_AMOUNT_PAID, 38, 6) > 0;
-- WHERE TRY_TO_NUMBER(REQ_NR_OTHER_PAYER_PATIENT_RESPONSIBILITY_AMOUNT_COUNT) > 0;
WHERE RES_5F_APPROVED_MESSAGE_CODE_COUNT > 0;



-- SELECT
--     LOG_RES_APPROVED_MESSAGE_CODE
--    ,TRANSFORM(STRTOK_TO_ARRAY(LOG_RES_APPROVED_MESSAGE_CODE, x -> TRIM(x))) AS ARRAY       //RESPONSE STATUS
-- FROM SAMPLE_DATA
-- WHERE LOG_RES_APPROVED_MESSAGE_CODE IS NOT NULL  AND LOG_RES_APPROVED_MESSAGE_CODE <> '' LIMIT 10;
--

SELECT DATE(CPH_INGESTED_TIMESTAMP) as dt, COUNT(*) as CNT FROM  CPE_PROD.DATA.CPE_CLAIMS_LOG GROUP BY dt;


SELECT
    (SELECT COUNT(*) FROM CPE_PROD.DATA.CPE_CLAIM_RESPONSES WHERE OTHER_PAYER_AMOUNT_PAID IS NOT NULL) as CNT_OTHER_PAYER_AMOUNT_PAID
    ,(SELECT COUNT(*) FROM CPE_PROD.DATA.CPE_CLAIM_RESPONSES WHERE NULLIF(COORDINATION_OF_BENEFITS_COUNT, '') IS NOT NULL) as CNT_COORDINATION_OF_BENEFITS_COUNT
 ;


SELECT COUNT(*) FROM CPE_PROD.DATA.POS_FILL_COMPLETED;
ALTER TASK CPE_PROD.DATA.STREAM_TASK_POS_FILL_COMPLETED SUSPEND;



select COUNT(*) from CPE_PROD.data.trx_cpesn_claim_events;
